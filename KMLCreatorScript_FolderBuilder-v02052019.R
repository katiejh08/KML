...
source("FolderBuilder_track2kml_fxns.R") #Standard KML is created.
#source("FolderBuilder_track2kml_fxns_NoTrackLines.R") #this creates a KML without the track lines
...
pointsize <- 0.3 # default value is 1
...
for(i in 1:length(unique(datax$name))){
    ...
    ss <- ss[order(ss$dttz),]	## data must be ordered on time before generating a kml! ##
    NONAME.track2kmlAddFolder(output.filename= fname, descriptive.foldername= unique(datax$name)[i])
    for(j in 1:length(unique(ss$yr))){
      ss1 <- subset(ss, yr == unique(ss$yr)[j])
      #ss1 <- ss1[order(ss1$dttz),]	## data must be ordered on time before generating a kml! ##
      NONAME.track2kmlAppendData(
        # Can?t touch this:
        latitude=ss1$lat, 
        ...
        col.scheme = ss1$colour,
        # Make the box:
        data.variables=data.frame(ss1$name, ss1$yr),# generates an attribute table for every GPS fix including the variables specified here
        # Specify the foldername:
        output.filename=fname,descriptive.foldername=unique(ss$yr)[j])
    }
    NONAME.track2kmlCloseFolder(output.filename= fname) # close the year folder
  }
